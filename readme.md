...
```bash
curl -sS https://raw.githubusercontent.com/make-repeat/atomic-deploy/main/atomic-deploy.php | php -- --revision=$(date "+%F-%H-%M-%S")
```
...
```
curl -sS https://raw.githubusercontent.com/make-repeat/atomic-deploy/main/atomic-deploy.php | php -- --revision=${execution.to_revision.revision} --symlinks='{"shared/config/.env.php":".env.php","shared/storage":"craft/storage"}'
```
